        ...
        if is_authenticated(request) == None:
            ...
            if request.GET.get('type') != 'buses':
                ...
                for booking in bus_bookings:
                    date = datetime.datetime.strptime(booking.get('To'), "%Y-%m-%d").date()
                    if date < datetime.date.today():
                        continue
                    else:
                        ...
                        upcoming_bookings.append(booking)
                return render(request, self.template_name, {'bus_bookings': upcoming_bookings, 'type': get_type(request)})
            ...
            if request.GET.get('type') != 'buses':
                ...
                for booking in bus_bookings:
                    ...
                    if date >= datetime.date.today():
                        ...
                        booking.update({'service_name': bus_detail.name})
                        past_bookings.append(booking)
                return render(request, self.template_name_1, {'bus_bookings': past_bookings, 'type': get_type(request)})
                


class BookingDetailView(View):

    template_name = 'booking/details_hotel.html'
            ...
            if id[0] == 'B':
                ...
                if not booking_details:
                    ...
                    if booking.get('email') != request.session.get('email'):
                        ...
                        return render(request, self.template_name_1, {'form': form, 'booking': booking, 'bus': bus, 'type': get_type(request)})                        
            ...
            if form.is_valid():
                ...
                if check_password(form.cleaned_data.get('password'), user.get('password')) == True:
                    ...
                    if r == 200:
                        ...
                        return render(request, self.template_name_1, {'form': forms.DeleteBookingForm(), 'error': '1', 'msg': 'Network Error', 'booking': booking, 'bus': bus, 'type': get_type(request)})
                else:
                    return render(request, self.template_name_1, {'form': forms.DeleteBookingForm(), 'error': '1', 'msg': 'Incorrect Password', 'booking': booking, 'bus': bus, 'type': get_type(request)})
            else:
                return render(request, self.template_name_1, {'form': forms.DeleteBookingForm(), 'error': '1', 'msg': 'Fields cannot be empty', 'booking': booking, 'bus': bus, 'type': get_type(request)})
